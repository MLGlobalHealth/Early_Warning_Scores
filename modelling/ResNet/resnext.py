                ...
                nn.ModuleDict(
                    {
                        ...
                        "linear0": nn.ModuleList(
                            [
                                nn.Linear(d, d_hidden_per_path)
                                for _ in range(cardinality)
                            ]
                        ),
                        "linear1": nn.ModuleList(
                            [
                                nn.Linear(d_hidden_per_path, d)
                                for _ in range(cardinality)
                            ]
                        ),
                    }
        ...
        for layer in self.layers:
            ...
            for i in range(self.cardinality):
                ...
                if self.hidden_dropout:
                    path_output = F.dropout(
                        path_output, p=self.hidden_dropout, training=self.training
                    )
                path_output = layer["linear1"][i](path_output)
                if self.residual_dropout:
                    path_output = F.dropout(
                        path_output, self.residual_dropout, self.training
                    )
